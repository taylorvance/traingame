import { createProjectStorage, type ProjectStorage } from '@taylorvance/tv-shared-runtime/storage'
import {
  createGame,
  normalizeRules,
  type BoardShape,
...
const BOARD_SHAPES: BoardShape[] = ['symmetric', 'asymmetric-left', 'asymmetric-right']
const COLORS: Color[] = ['red', 'blue']
const GAME_STATUSES: GameStatus[] = ['playing', 'won', 'lost']
const TILE_KINDS: TileKind[] = ['straight', 'softLeft', 'softRight', 'hardLeft', 'hardRight']
...
function isBoardShape(value: unknown): value is BoardShape {
  return typeof value === 'string' && BOARD_SHAPES.includes(value as BoardShape)
}
...
function isTile(value: unknown): boolean {
  return isRecord(value) && typeof value.id === 'string' && isTileKind(value.kind)
}
...
function normalizeSavedRules(value: unknown): RulesConfig | null {
  return isRulesConfig(value) ? normalizeRules(value) : null
}

export function loadPersistedAppState(storage: AppStateReader = APP_STORAGE): PersistedAppState | null {
...
export function savePersistedAppState(state: PersistedAppState, storage: AppStateWriter = APP_STORAGE): void {
  storage.writeJson(state, APP_STATE_KEY)
}

export function clearPersistedAppState(storage: AppStateWriter = APP_STORAGE): void {
